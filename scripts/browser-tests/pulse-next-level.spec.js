...
test.describe('Pulse next-level contracts — proposal workspace', () => {
  ...
  test('evidência abre Inspector com origem, período, confiança e ações', async ({page})=>{
    await page.goto('/index.html#overview');
    ...
    await expect(ins).toContainText(expected.fonte);await expect(ins).toContainText(expected.periodo);await expect(ins).toContainText(expected.confianca);
