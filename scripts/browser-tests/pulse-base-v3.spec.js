...
test.describe('Base e Análises: reprojeção visual',()=>{
 ...
 test('Análises é destino distinto e preserva a mesma linguagem de workspace',async({page})=>{await page.goto('/scripts/browser-tests/fixtures/app.html#analyses');await expect(page.locator('.pv4-app')).toBeVisible();await expect(page.locator('.pv4-base-hero')).toContainText('O que já virou conhecimento de trabalho?');});
});