...
# Restore navigation controller from main if absent/stranded.
...
# Restore custom Ask AI renderer from main if absent.
...
# Move history functions before the synchronous enhancement IIFE.
...
# Avoid duplicate insertion by removing any prior copies left before marker.
s=s.replace('\n\n'.join(history_blocks)+'\n\n','\n',1)
s=s.replace(marker,'<script id="pulse-history-runtime">\n'+'\n\n'.join(history_blocks)+'\n</script>\n\n'+marker,1)

# Replace cumulative keyword scoring inside pulseMatchEvidence with best-match-per-item scoring.
