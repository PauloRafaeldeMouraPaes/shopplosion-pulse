...
(function () {
  ...
  function categoryVisibility() {
    var hash = location.hash || '#overview', picker = q('.pulse-category-picker') || q('#pulse-category-select')?.parentElement;
  ...
  function markLocalResults() {
    qa('[data-pulse-local-result]').forEach(function (el) { el.classList.add('pulse-source-local'); });
  }
  function installSuggestionHandler() {
    ...
    suggestions.forEach(function (a) {
      ...
      clone.addEventListener('click', function (e) { e.preventDefault(); suggestions.forEach(function (s) { s.classList.remove('selected'); }); clone.classList.add('selected'); input.value = clone.dataset.question || ''; input.focus(); submit.setAttribute('href', clone.dataset.target || '#answer-custom'); });
    });
  }
  function init() {
    ...
    var obs = new MutationObserver(function () { updateCategoryOptions(); markLocalResults(); fixPrimaryJourneyCTA(); });
