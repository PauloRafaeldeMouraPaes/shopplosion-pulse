...
Deno.serve(async (req) => {
  ...
  // Tenant scope is enforced by the authenticated Supabase session + RLS.
  // Do not infer cross-industry access from natural-language phrases such as
  // "minha indústria" or from model wording. Those heuristics caused false
  // positives and are not a security boundary.
  const filters = terms.map((term) => `content.ilike.%${term}%`).join(',')
  const { data: chunks, error: chunkError } = await supabase
    .from('document_chunks')
    .select('id,document_id,industry_id,chunk_index,content,source_type,created_at')
  ...
  // Defense in depth: even if a future query path changes, never pass a chunk
  // outside the authenticated tenant to the model.
  const tenantChunks = (chunks || []).filter((chunk) => String(chunk.industry_id) === String(profile.industry_id))
  ...
  const ranked = tenantChunks.map((chunk) => ({ ...chunk, score: score(chunk.content || '') })).filter((chunk) => chunk.score > 0).sort((a, b) => b.score - a.score || a.chunk_index - b.chunk_index).slice(0, 8)
  ...
  const { data: documents, error: documentError } = await supabase
    .from('documents')
    .select('id,filename,industry_id')
    .eq('industry_id', profile.industry_id)
    .in('id', documentIds)
  ...
  const evidence = ranked
    .filter((chunk) => documentMap[chunk.document_id] && String(documentMap[chunk.document_id].industry_id) === String(profile.industry_id))
    .map((chunk, index) => ({ ref: `E${index + 1}`, document: documentMap[chunk.document_id]?.filename || 'Documento privado', chunk: Number(chunk.chunk_index) + 1, source_type: chunk.source_type || 'text', content: String(chunk.content || '').slice(0, 5000) }))

  if (!evidence.length) return json({ answer: 'Não encontrei evidências relevantes na base privada da sua indústria.', citations: [] })
  ...
  const system = 'Você é o analista privado do Shopplosion Pulse. Responda em português do Brasil, de forma objetiva e analítica. Use SOMENTE as evidências fornecidas. Não invente números, fatos, fontes ou conclusões. Diferencie claramente FACT e INFERENCE quando houver inferência. Se as evidências não sustentarem a resposta, diga isso. Sempre cite as evidências usadas no formato [E1], [E2]. A única informação privada disponível nesta chamada é a contida nas evidências fornecidas, que já foram filtradas pela indústria autenticada. Se a pergunta pedir dados de outra indústria e essas evidências não os sustentarem, diga que não há evidências disponíveis, sem tentar inferir ou inventar dados. Não transforme frases genéricas como "minha indústria", "sua indústria" ou "indústria autenticada" em nomes de outras indústrias.'
